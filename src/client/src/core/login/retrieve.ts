...
const hasUser = async (user: string): Promise<boolean> => {
    ...
    try {
        const res = await axios.get('/login/hasUser', params) as { res: number }
...
const sendVerifyCode = async (user: string): Promise<boolean> => {
    ...
    try {
        const res = await axios.get('/login/sendVerifyCode', params) as { res: number, code: string, email: string }
...
const recover = async (form: form): Promise<number> => {
    try {
        const res: { res: number } = await axios.post('/login/recover', {
            ...form,
            pswd: md5(form.pswd)
        }) as { res: number }
        return res.res
