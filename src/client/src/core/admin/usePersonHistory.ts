...
export default function () {
    ...
    const getList = async (page: number) => {
        try {
            const res = await axios.get('admin/invitation/getAdminHistoryList', { page }) as { res: number, pSum: number, adminInvitations: invitation[] }
            if (res.res == 1) {
    ...
    const deleteInvitation = async (iid: string) => {
        try {
            const res = await axios.post('admin/invitation/deleteAdminHistory', { iid }) as { res: number }
            if (res.res == 1) {
