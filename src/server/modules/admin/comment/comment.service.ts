...
import { postCommentRes, reply, replyRes } from './dto/comment.dto';
...
export class CommentService {
    ...
    async postComment(uid1: string, iid: string, text: string): Promise<postCommentRes> {
        ...
        try {
            ...
            const { floor }: { floor: number } = await this.commitRepository.createQueryBuilder().select("MAX(commit.floor)", "floor").getRawOne()
            const date = new Date()
