...
export class UploadService {
    ...
    async uploadImage(file: Express.Multer.File): Promise<uploadImageRes> {
        ...
        try {
            ...
            const id = createId()
            const date = new Date()
            const path = join(__dirname, '../../public/image', `${id + '-' + name}`)
            const url = `http://localhost:3000/static/image/${id + '-' + name}`
            const writeSteam = createWriteStream(path)
            await new Promise<void>((resolve, reject) => writeSteam.write(file.buffer, (Err) => {
                if (Err) reject(Err)
                resolve()
            }))
            const insertRes: InsertResult = await this.uploadRepository.insert({ id, name, date, path, url })
    ...
    async uploadVideo(file: Express.Multer.File): Promise<uploadVideoRes> {
        ...
        try {
            ...
            const id = createId()
            const date = new Date()
            const path = join(__dirname, '../../public/video', `${id + '-' + name}`)
            const url = `http://localhost:3000/static/video/${id + '-' + name}`
            const writeSteam = createWriteStream(path)
            await new Promise<void>((resolve, reject) => writeSteam.write(file.buffer, (Err) => {
                if (Err) reject(Err)
                resolve()
